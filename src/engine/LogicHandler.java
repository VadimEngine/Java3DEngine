...
public class LogicHandler {
    ...
    public void render(RenderHandler renderer) {
        ...
        /*
         * Coordinate cc1 = new Coordinate(0,0,scale); Coordinate cc2 = new
         * Coordinate(300,300,scale); Coordinate cc3 = new Coordinate(0,300,scale);
         * Coordinate cc4 = new Coordinate(300,0,scale);
         * 
         * cc1 = Calculator.rotateAroundCamera(cc1, theCamera); cc2 =
         * Calculator.rotateAroundCamera(cc2, theCamera); cc3 =
         * Calculator.rotateAroundCamera(cc3, theCamera); cc4 =
         * Calculator.rotateAroundCamera(cc4, theCamera);
         * 
         * VertexTex v0 = new VertexTex(cc1.getX(), cc1.getY(), cc1.getZ(), 0, 0,
         * theTexture); VertexTex v1 = new VertexTex(cc2.getX(), cc2.getY(), cc2.getZ(),
         * 1, 1, theTexture); VertexTex v2 = new VertexTex(cc3.getX(), cc3.getY(),
         * cc2.getZ(), 0, 1, theTexture); VertexTex v3 = new VertexTex(cc4.getX(),
         * cc4.getY(), cc4.getZ(), 1, 0, theTexture);
         */
    ...
    public void selectMesh(int xPos, int yPos) {
        ...
        for (int j = 0; j < objectList.size(); j++) {
            ...
            for (int i = 0; i < meshIndices.size(); i += 3) {
                ...
                c1 = Calculator.rotateAroundCenter(c1, new Coordinate(0, 0, 0), eachObj.getXAngle(),
                        eachObj.getYAngle(), eachObj.getXAngle());
                c2 = Calculator.rotateAroundCenter(c2, new Coordinate(0, 0, 0), eachObj.getXAngle(),
                        eachObj.getYAngle(), eachObj.getXAngle());
                c3 = Calculator.rotateAroundCenter(c3, new Coordinate(0, 0, 0), eachObj.getXAngle(),
                        eachObj.getYAngle(), eachObj.getXAngle());
    ...
    /**
     * Draws a rectangle at the same x, y of the screen but at z=0, works pretty
     * well only if camera moves and only XY angles changes, but re-adjusting the
     * mouse once the other angles change keeps the shape drawn at the same location
     * on screen, Need to take the other angles more into account. Also need to
     * rotate the other angles so the shape is always drawn parallel/perpendicular
     * to the camera direction //does near need to be taken into account?
     * 
    ...
    private void addManMesh() {
        ...
        objectList.add(
                new Object3D(Mesh.createCubeMesh(-25 + 4, -10 + 4, 300, 100 - 4, 80 - 4, 100, woodColor), woodColor)); // chest
        objectList.add(
                new Object3D(Mesh.createCubeMesh(-25 + 4, -10 + 4, 250, 100 - 4, 80 - 4, 50, woodColor), woodColor)); // waist
        objectList.add(
                new Object3D(Mesh.createCubeMesh(-25 + 4, -10 + 4, 200, 100 - 4, 80 - 4, 50, woodColor), woodColor)); // hip
