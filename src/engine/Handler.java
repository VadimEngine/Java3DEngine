...
/**
 * Key and mouse Listener. Saves the keys that are pressed in a boolean array
 * and also saves the mouse Mouse position
 * 
 * 
 * Instead of using the listener to do actions in the game (Which may cause the
 * tick to not be synced), it might be like an extra thread messing with
 * game/data flow, Save the mouse info like mouse x,y, left/right/middle
 * boolean, isDrag? Too much game logic in the mouse action listeners. Have a
 * previous keys/mouse information saved so it can be used in game logic
 * 
 * @author Vadim
 *
 */
