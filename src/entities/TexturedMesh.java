...
public class TexturedMesh {
    ...
    public static TexturedMesh createTextureCube(double x, double y, double z, double width, double depth,
            double height, Texture theTexture) {

        List<VertexTex> coords = new ArrayList<>();
        List<Integer> indices = new ArrayList<>();

        coords.add(new VertexTex(x, y, z, 0, 0, theTexture));
        coords.add(new VertexTex(x + width, y, z, 1, 0, theTexture));
        coords.add(new VertexTex(x + width, y + depth, z, 1, 1, theTexture));
        coords.add(new VertexTex(x, y + depth, z, 0, 1, theTexture));

        coords.add(new VertexTex(x, y, z + height, 0, 0, theTexture));
        coords.add(new VertexTex(x + width, y, z + height, 1, 0, theTexture));
        coords.add(new VertexTex(x + width, y + depth, z + height, 1, 1, theTexture));
        coords.add(new VertexTex(x, y + depth, z + height, 0, 1, theTexture));
